...
IP-ISAT = 0 # whether to perform IP-ISAT training
SEP-ISAT = 1 # whether to perform SEP-ISAT training
...
def my_loss_H(output, target, X, nClusters, nKmeans): # custom loss function for SEP-ISAT preconditioning
    ...
    for ii in range(nClusters):        
        ...
        grad = torch.linalg.lstsq(X[ii*nKmeans:(ii+1)*nKmeans,:], output1[ii*nKmeans:(ii+1)*nKmeans,:],driver = 'gelsd').solution
    
        output1[ii*nKmeans:(ii+1)*nKmeans,:] = output1[ii*nKmeans:(ii+1)*nKmeans,:] - torch.matmul(X[ii*nKmeans:(ii+1)*nKmeans,:],grad).detach()
